...
public class AccountDAO {
	...
	public static int getPlayerAccess(String playerName, String hashPass) {
		...
		PreparedStatement st = DB.prepareStatement(
				"SELECT access_level FROM players P, account_data AD WHERE P.account_name = AD.name AND P.name = ? AND AD.password = ?");
	...
	/**
	 * Insert new account to database. Sets account ID to id that was generated by
	 * DB.
	 *
	 * @param account
	 *            account to insert
	...
	public static boolean updateAccount(Account account) {
		int result = 0;
		PreparedStatement st = DB.prepareStatement(
				"UPDATE account_data SET `name` = ?, `password` = ?, access_level = ?, membership = ?, last_server = ?, last_ip = ?, last_mac = ?, ip_force = ?, membership_expire=? WHERE `id` = ?");
