...
public class PlayerTransferService {
	...
	protected void verifyNewTasks() {
		...
		for (PlayerTransferTask task : tasksNew) {
			...
			if (server == null || server.getConnection() == null) {
				log.error("cannot perform transfer task #" + task.id + " while source server is down #"
						+ task.sourceServerId);
				continue;
			}
			...
			if (targetServer == null || targetServer.getConnection() == null) {
				log.error("cannot perform transfer task #" + task.id + " while target server is down #"
						+ task.targetServerId);
				continue;
			}

			if (server.isAccountOnGameServer(task.sourceAccountId)) {
				log.error("cannot perform transfer task #" + task.id + " while source account is online "
						+ task.sourceAccountId);
				continue;
			}

			if (targetServer.isAccountOnGameServer(task.targetAccountId)) {
				log.error("cannot perform transfer task #" + task.id + " while target account is online "
			...
			server.getConnection()
					.sendPacket(new SM_PTRANSFER_RESPONSE(PlayerTransferResultStatus.PERFORM_ACTION, task));
	...
	public void requestTransfer(int taskId, String name, byte[] db) {
		...
		if (targetServer.isAccountOnGameServer(task.targetAccountId)) {
			log.error("Player transfer cant be performed while target account is online at server #"
					+ task.targetServerId + ". " + task.targetAccountId);
		...
		if (transfers.containsKey(taskId)) {
			log.error("Player transfer cant be performed while it is already active #" + task.targetServerId + ". "
					+ task.targetAccountId);
			server.getConnection().sendPacket(new SM_PTRANSFER_RESPONSE(PlayerTransferResultStatus.ERROR, taskId,
					"transfer cant be performed while it is already active"));
			return;
		}
		...
		targetServer.getConnection()
				.sendPacket(new SM_PTRANSFER_RESPONSE(PlayerTransferResultStatus.SEND_INFO, request));
	...
	public void onError(int taskId, String reason) {
		...
		targetServer.getConnection()
				.sendPacket(new SM_PTRANSFER_RESPONSE(PlayerTransferResultStatus.ERROR, taskId, reason));
	}
