...
public class CmdRevenge extends BaseCommand {
		...
		ThreadPoolManager.getInstance().schedule(new Runnable() {
			...
			public void run() {
				...
				if (player.getLifeStats().isAlreadyDead()) {
					...
					adminscore += 1;
					MessagerAddition.announce(player,
							"[Revenge] You lost this battle. Player " + admin.getName() + " gets one point");
					MessagerAddition.announce(admin,
							"[Revenge] Congratulations, you won, you get one point to your account");
					admIsWin1 = true;
					restore(player);
					restore(admin);
					TeleportService.teleportTo(player, 510010000, 256, 256, 49, 3000, true);
					...
					MessagerAddition.announce(player,
							"[Revenge] Congratulations, you won, you get one point to your account");
					MessagerAddition.announce(admin,
							"[Revenge] You lost this battle. Player " + player.getName() + " gets one point");
					playerIsWin1 = true;
					restore(player);
					restore(admin);
					TeleportService.teleportTo(player, 510010000, 256, 256, 49, 3000, true);
	...
	private void onEnd(final Player player, final Player admin) {
		MessagerAddition.announceAll(
				"[Revenge] And the results of the battle " + admin.getName() + " vs " + player.getName(), 0);
		ThreadPoolManager.getInstance().schedule(new Runnable() {
			...
			public void run() {
				...
				if (adminscore == playerscore) {
					MessagerAddition.announceAll("[Revenge] And the results of the battle " + admin.getName() + " vs "
							+ player.getName() + " <DRAW> !", 0);
				}
				if (adminscore > playerscore) {
					MessagerAddition.announceAll("[Revenge] And the results of the battle " + admin.getName() + " vs "
							+ player.getName() + " - " + admin.getName() + " wins!", 0);
				} else
					MessagerAddition.announceAll("[Revenge] And the results of the battle " + admin.getName() + "vs"
							+ player.getName() + " - " + player.getName() + " wins!", 0);
