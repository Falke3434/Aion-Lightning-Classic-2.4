...
public enum AbnormalState {
	...
	CURSE(1024), CHAOS(2056), STUN(4096, 2), PETRIFICATION(8192), STUMBLE(16384, 1), STAGGER(32768,
			1), OPENAERIAL(65536, 1), SNARE(131072), SLOW(262144), SPIN(524288, 1), BIND(1048576), DEFORM(2097152), // (Curse
																													// of
																													// Roots
																													// I,
																													// Fear
	...
	CANT_ATTACK_STATE(SPIN.id | SLEEP.id | STUN.id | STUMBLE.id | STAGGER.id | OPENAERIAL.id | PARALYZE.id | FEAR.id
			| CANNOT_MOVE.id), CANT_MOVE_STATE(
					SPIN.id | ROOT.id | SLEEP.id | STUMBLE.id | STUN.id | STAGGER.id | OPENAERIAL.id | PARALYZE.id
							| CANNOT_MOVE.id);
